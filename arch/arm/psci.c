...
{
    arm_smccc_smc(PSCI_FN_CPU_SUSPEND, state, entry_point, 0, 0, 0, 0, 0, NULL);
}

void psci_cpu_off(uint32_t state)
{
    arm_smccc_smc(PSCI_FN_CPU_OFF, state, 0, 0, 0, 0, 0, 0, NULL);
}

void psci_cpu_on(unsigned long cpu_id, unsigned long entry_point)
{
    arm_smccc_smc(PSCI_FN_CPU_ON, cpu_id, entry_point, 0, 0, 0, 0, 0, NULL);
}

void psci_migrate(unsigned long cpu_id)
{
    arm_smccc_smc(PSCI_FN_MIGRATE, cpu_id, 0, 0, 0, 0, 0, 0, NULL);
}
